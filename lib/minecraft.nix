        ...
        in
        {
          containers.${srvName} = mkIf config.srv.${srvName}.enable {
            ...
            forwardPorts = # (mkIf config.srv.${srvName}.forward-ports.enable)
              (
                attrValues (
                  genAttrs (builtins.attrNames net.ports) (portName: {
                    ...
                    ${srvName} = mkMerge [
                      {
                        ...
                        package = srvPackage;
                        serverProperties = {
                          server-port = net.ports.minecraft;
                        };
                        symlinks = {
