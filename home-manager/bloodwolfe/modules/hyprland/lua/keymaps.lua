...
util.mkNormalSubmap("MOVE_ACTIVE_WINDOW_TO_WORKSPACE", function()
	for key, val in pairs(key_workspace_maps) do
		hl.bind(key, hl.dsp.window.move({ workspace = val, window = activewindow }))
	end
end)
...
util.mkNormalSubmap("UTIL", function()
	...
	hl.bind("p", hl.dsp.exec_cmd('hyprpicker -a -f hex | xargs -I {} notify-send "{}"'))
	hl.bind(
		"c",
		hl.dsp.exec_cmd(
			'pgrep -x .wl-crosshair-w >/dev/null && pkill -x .wl-crosshair-w || wl-crosshair "$FLAKE/assets/crosshairs/crosshair_magenta_2px.png"'
		)
	)
	hl.bind(
		"w",
